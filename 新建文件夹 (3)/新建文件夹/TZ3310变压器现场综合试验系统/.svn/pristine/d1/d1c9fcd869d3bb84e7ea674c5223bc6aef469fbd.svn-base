...
namespace SCEEC.NET
{
        ...
        SerialClass
    {
         ...
         public void _seriaPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
               ...
               //System.Threading.Thread.Sleep(20);
                ...
                //  string  data = _seriaPort.ReadExisting();
                if (_data.Length == 0)
                {
                    return;
                }
        ...
        public ArrayList ReadLargeData(byte[] sendConmand)
        {
            ...
            if (_seriaPort.IsOpen)
            {
                ReceiveEventFlag = true;//关闭接收事件
                _seriaPort.DiscardInBuffer();//清空接收缓冲区     
                _seriaPort.Write(sendConmand, 0, sendConmand.Length);
                // int num = 0, ret = 0;
                Thread.Sleep(500);
                ReceiveEventFlag = false;//打开事件
                try
                {
                    while (true)
                    {
                       //System.Threading.Thread.Sleep(500);
                        ...
                        int listLeng = _seriaPort.BytesToRead;
                        if (listLeng <=0)
                            break;
                       Thread.Sleep(500);
                        byte[] newbye = new byte[listLeng];
                        _seriaPort.Read(newbye, 0, listLeng);
                        //_seriaPort.
